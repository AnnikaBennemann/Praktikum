...
plt.errorbar(d, np.log(Nt0), yerr=[np.log(Nt0+sigmaNt0)-np.log(Nt0), np.log(Nt0)-np.log(Nt0-sigmaNt0)], fmt = 'o',color='r', markersize=2, capsize=2, ecolor='b', elinewidth=0.5, markeredgewidth=0.5, label='Messwerte mit Fehlerbalken')
...
print(params)
print(np.sqrt(np.diag(covariance_matrix)))
plt.errorbar(db, np.log(Ntb0), yerr=[np.log(Ntb0+sigmaNtb0)-np.log(Ntb0), np.log(Ntb0)-np.log(Ntb0-sigmaNtb0)], fmt = 'o',color='r', markersize=2, capsize=2, ecolor='b', elinewidth=0.5, markeredgewidth=0.5, label='Messwerte mit Fehlerbalken')
plt.gcf().subplots_adjust(bottom=0.18)
...
Aer2= np.delete(Aerror,[0,1,2,3,4,5])
...
plt.errorbar(d_plot2, np.log(np.abs(A_plot2)), yerr=[np.log(np.abs(A_plot2)+Aer2)-np.log(np.abs(A_plot2)), np.log(np.abs(A_plot2))-np.log(np.abs(A_plot2)-Aer2)], fmt = 'o',color='b', markersize=2, capsize=2, ecolor='r', elinewidth=0.5, markeredgewidth=0.5, label='Messdaten mit Fehlerbalken')
plt.plot(d_plot2, f(d_plot2, *params), 'cyan', label='Ausgleichsgerade', linewidth=1.5)
errors = np.sqrt(np.diag(cov))
plt.legend()
plt.grid()
plt.ylabel(r'$\ln(A-A_U)$')
