...
Nun kann aus \autoref{fig:Abb_3} die Viskosität der Luft für die Temperatur abgelesen werden.
...
\begin{table}[H]
        ...
        \toprule
        {$R / \si{\mega\ohm}$}& {$T / \si{\celsius}$} &{$\eta_{Luft} / 10^{-5} \frac{\si{\newton\second}}{\si{\meter\squared}}$} & \multicolumn{2}{c}{$r / 10^{-6} \si{\meter}$} & \multicolumn{2}{c}{$q / 10^{-19}\si{\coulomb}$} & \multicolumn{2}{c}{$q_{korr} / 10^{-19}\si{\coulomb}$} \\
        \midrule
        1.992 & 25.60 & 1.8497 & 0.70 & 0.03 & 6.15 & 0.34 &  7.25 & 0.40 \\   
...
Um aus der Ladung die Elementarladung $e_0$ zu ermitteln werden zunächst die berechneten Ladungen in \autoref{fig:e_01} und \autoref{fig:e_02} graphisch dargestellt.
