...
Die Werte, die mit der Gegenfeldmethode aufgenommen werden, werden in \autoref{fig:plot1b} dargestellt, wobei die Wurzel des Photonenstroms
gegenüber der Spannung aufgetragen wird. Außerdem wird mit den Pythonmodulen Matplotlib\cite{matplotlib},  Uncertainties\cite{uncertainties} und Numpy\cite{numpy}
eine Ausgleichsgerade erstellt. Die Parameter der Ausgleichsgerade werden in \autoref{tab:Frequenzen} eingefügt.
\begin{figure}[H]
  \centering
...
In \autoref{fig:plot2} wird die Wurzel der Stromstärke gegenüber der der Spannung für grünes Licht dargestellt.
...
\autoref{fig:plot5} mit einer Ausgleichsgerade dargestellt. Die berechneten Parameter der Ausgleichgerade werden in \autoref{tab:Frequenzen}
eingefügt.
\begin{table}[H]
  \centering
...
Aus den gegebenen Wellenlängen(\cite[80]{V500}) werden die Frequenzen für verschiedene Farben mit \autoref{eqn:Frequenz} berechnet
