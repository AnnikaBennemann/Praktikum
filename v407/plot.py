...
alpha_s, I_s = np.genfromtxt("content/senkrecht.txt", unpack = True)
alpha_p, I_p = np.genfromtxt("content/parallel.txt", unpack = True)
...
# Mean
n_s_mean = np.mean(n_s)
n_s_err = ufloat(n_s_mean, np.std(n_s))
