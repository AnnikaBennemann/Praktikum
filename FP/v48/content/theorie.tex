...
Da die Dipole sich bei genügend hoher Temperatur wieder statistisch ausrichten und dies mit Ladungsträgerdiffusion und einer Wanderung
von Elektronen durch die Kondensatorplatten verbunden ist,
entsteht ein elektrisch messbarer Strom.
Der Strom wird Depolarisationsstrom genannt und die Stromdichte ist gleich der Änderungsrate der 
...
\end{equation}
Ableiten dieser Lösung nach der Zeit ergibt wieder die Änderung der Polarisation und somit die
Depolarisationsstromdichte.
\begin{equation}
    j(T) = \frac{P_0}{\tau(T)} \text{exp}\left(-\int_0^t\frac{\text{d}t}{\tau(T)}\right) \quad.
    \end{equation}
Mit der konstanten Heizrate
\begin{equation}
    b := \frac{\text{d}T}{\text{d}t} = const
\end{equation}
lässt sich die Depolarisationsstromdichte nun als 
\begin{equation}
    j(T) = \frac{P_0}{\tau(T)} \text{exp}\left(-\frac{1}{b\tau_0}\int_{T_0}^T\frac{\text{d}T'}{\tau(T')}\right)
\end{equation}
...
\subsubsection{Verwendung des gesamten Kurvenverlaufs}
        ...
        \begin{equation}
            \tau(T) = \frac{\int \frac{dP}{dt}\text{d}T}{j(T)b} \quad .
        \end{equation}
        Die Stromdichte kann hier erneut gegen die Änderung der Polarisation vertauscht werden
        \begin{equation}
            \tau(T) = \frac{\int_T^\infty j(T')\text{d}T'}{j(T)b}
        \end{equation}
